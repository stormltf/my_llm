...
class BPETokenizer:
    ...
    def fit(self, texts: List[str], verbose: bool = True) -> 'BPETokenizer':
        ...
        核心思想：
        ---------
        从字符级别开始，反复合并最高频的相邻字符对，逐步构建子词词表。

        为什么使用 BPE？
        ---------------
        1. 平衡词粒度：既不是纯字符（太长），也不是纯词（OOV问题）
        2. 高效压缩：常见组合变成单个 token，减少序列长度
        3. 处理生僻词：通过子词组合处理未见过的词

        算法步骤（图示）：
        -----------------
        假设训练语料："low" 出现 5 次，"lower" 出现 2 次，"newest" 出现 6 次

        ┌──────────────────────────────────────────────────────────────────────────┐
        │ 初始状态（字符级别）                                                      │
        │ ┌────────────────────────────────────────────────────────────────┐       │
        │ │ 词表: {'l':0, 'o':1, 'w':2, 'e':3, 'r':4, 'n':5, 's':6, 't':7} │       │
        │ └────────────────────────────────────────────────────────────────┘       │
        │ 分词状态:                                                                 │
        │   "low"    → [l, o, w]       × 5                                         │
        │   "lower"  → [l, o, w, e, r] × 2                                         │
        ...
        │                                                                          │
        │ 最终可能得到的 token:                                                     │
        │   "low", "lower", "new", "newest", "est" 等                              │
        │                                                                          │
        │ 合并规则记录:                                                             │
        │   merges = [(w, e), (l, o), (lo, w), (n, e), (ne, w), ...]               │
        └──────────────────────────────────────────────────────────────────────────┘
    ...
    def _apply_bpe(self, word: str) -> List[str]:
        ...
        这是 BPE 编码的核心：将训练时学到的合并规则应用到新词上。
        ...
        为什么顺序重要？
        --------------
        训练时，先学到的合并规则代表更常见的组合。
        例如，假设我们有合并规则：
            merges = [('l', 'o'), ('lo', 'w')]

        对于单词 "low"：
        - 正确顺序：
            [l, o, w] → 应用('l','o') → [lo, w] → 应用('lo','w') → [low]

        - 错误顺序（如果先尝试 ('lo','w')）：
            [l, o, w] → 找不到 'lo'（因为还没合并）→ 无法继续

        示例：编码 "lower"
        -----------------
        假设 merges = [('e', 'r'), ('l', 'o'), ('lo', 'w'), ('w', 'e')]
        ...
        处理未知词（OOV）：
        -----------------
        如果某个字符/组合不在合并规则中，它会保持为字符级别的 token。
        例如，如果 "xyz" 从未在训练数据中出现：
            [x, y, z] → 没有可应用的规则 → 结果仍是 [x, y, z]

        这就是 BPE 能处理任意新词的原因：最坏情况下回退到字符级别。
        """
        if len(word) == 1:
            return [word]

        # ============================================================
        # Step 1: 初始化 - 将单词拆分为字符序列
        # ============================================================
        # "hello" → ['h', 'e', 'l', 'l', 'o']
...
class MyLLMTokenizer(BPETokenizer):
    ...
    def __init__(self, vocab_path: Optional[str] = None, vocab_size: int = 2000):
        """
        初始化分词器
    ...
    def build_vocab(self, text: str, max_vocab_size: int = 2000):
        """
        从文本构建词表（使用 BPE 算法）
...
def create_chinese_vocab(texts: List[str], vocab_size: int = 2000) -> MyLLMTokenizer:
    """
    从中文文本创建 BPE 分词器
